...
- **Vocal Separation / Local Whisper**: Fix bundled native tools (BS-Roformer vocal separation, whisper.cpp, CTC aligner, VAD) failing with "Failed to open file for writing" for users whose Windows username contains non-ASCII characters (Chinese, Japanese, Korean). These tools create their own scratch files under `%TEMP%`, whose path was corrupted by the CJK username; MioSub now points the child process's temp directory to an ASCII-safe location.
- **Settings**: Sanitize the API key inputs so a key pasted together with surrounding text (a note, a full-width space) no longer crashes generation with "String contains non ISO-8859-1 code point".
- **Settings**: Sanitize and validate the custom API endpoint inputs. Stray whitespace and a trailing `/v1` are now handled automatically (the latter was the single largest cause of translation/refinement failures on third-party proxies, producing a malformed `/v1/v1beta/...` request URL), and an invalid endpoint shows an inline warning.

## [3.1.8] - 2026-07-18

### Fixes

- **Whisper (Local)**: Fix all local transcription and VAD failing with cryptic exit code 3221225781 on machines without the Microsoft Visual C++ runtime (typically brand-new PCs). The bundled whisper.cpp binaries (v1.8.8) are now built with the static runtime and no longer require the VC++ Redistributable. If a binary still cannot load, MioSub shows an actionable message with the vc_redist download link instead of a raw exit code, and the binary updater now offers a replacement download for this case instead of staying silent (Fixes MIOSUB-7J, MIOSUB-7K).
- **Whisper (Local)**: Restore the `--version` flag in the bundled whisper.cpp build (lost in v1.8.7), so version detection and update checks work again. Binaries that run fine but cannot report a version are now offered an update instead of being stuck on "unknown" forever.
- **Pipeline**: Validate LLM JSON field types at the parse boundary — malformed glossary terms (nested objects) and non-string speaker names returned by third-party proxies no longer crash the glossary merger, the editor, or subtitle export; corrupted glossaries persisted by earlier versions are cleaned up on load (Fixes MIOSUB-76, MIOSUB-6Y, MIOSUB-78, MIOSUB-79).
